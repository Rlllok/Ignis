...
layout(location = 3) in vec2   center_position;
layout(location = 4) in vec2   size;
layout(location = 5) in float  is_box;
...
void main()
{
  ...
  if (is_box > 0.0f)
  {
    d = IG_SDF_Box(pixel - center_position, size);
  }
  else
  {
    d = IG_SDF_Circle(pixel - center_position, size.x);
  }

  vec3 final_color = red_color;
  // vec3 final_color = mix(vec3(gl_FragColor), red_color, 1.0f - smoothstep(0.0f, 3.0f, abs(d)));
