...
{
  RHI_COMPARE_OPERATION_EQUAL,
  RHI_COMPARE_OPERATION_NOT_EQUAL,
  RHI_COMPARE_OPERATION_LESS,
  RHI_COMPARE_OPERATION_LESS_OR_EQUAL,
  RHI_COMPARE_OPERATION_GREATER,
  RHI_COMPARE_OPERATION_GREATER_OR_EQUAL,

  RHI_COMPARE_OPERATION_COUNT
...
enum
{
  RHI_BUFFER_USAGE_FLAG_ZERO = 0,

  RHI_BUFFER_USAGE_FLAG_VERTEX  = 1 << 0,
  RHI_BUFFER_USAGE_FLAG_INDEX   = 1 << 1,
  RHI_BUFFER_USAGE_FLAG_UNIFORM = 1 << 2,
  RHI_BUFFER_USAGE_FLAG_TRANSFER = 1 << 3,

  RHI_BUFFER_USAGE_FLAG_COUNT
};
...
enum
{
  RHI_BUFFER_PROPERTY_FLAG_ZERO = 0,

  RHI_BUFFER_PROPERTY_FLAG_HOST_COHERENT = 1 << 0,
  RHI_BUFFER_PROPERTY_FLAG_HOST_VISIBLE = 1 << 1,
};
...
// -------------------------------------------------------------------
// Texture
typedef U8 RHI_TextureType;
enum RHI_TextureTypeEnum
{
  RHI_TEXTURE_TYPE_2D,
} RHI_TextureTypeEnum;
...
{
  RHI_TEXTURE_FORMAT_NONE,
  RHI_TEXTURE_FORMAT_R8G8B8A8_SRGB,
  RHI_TEXTURE_FORMAT_R8G8B8A8_UNORM,
  RHI_TEXTURE_FORMAT_B8G8R8A8_UNORM,
  RHI_TEXTURE_FORMAT_R16G16B16A16_SFLOAT,
  RHI_TEXTURE_FORMAT_D16_UNORM,
  RHI_TEXTURE_FORMAT_R16_UINT,
...
typedef U8 RHI_FilterType;
enum RHI_FilterTypeEnum
{
  RHI_FILTER_TYPE_NEAREST,
  RHI_FILTER_TYPE_LINEAR,
} RHI_FilterTypeEnum;

typedef U8 RHI_SamplerMipmapMode;
enum RHI_SamplerMipmapModeEnum
{
  RHI_SAMPLER_MIPMAP_MODE_NEAREST,
  RHI_SAMPLER_MIPMAP_MODE_LINEAR,
...
{
  RHI_FilterType mag_filter;
  RHI_FilterType min_filter;
...
{
	RHI_ATTACHMENT_LOAD_OPERATION_DONT_CARE,
	RHI_ATTACHMENT_LOAD_OPERATION_CLEAR,
	RHI_ATTACHMENT_LOAD_OPERATION_LOAD,
} RHI_LoadOperationEnum;

typedef U8 RHI_StoreOperation;
typedef enum RHI_StoreOperationEnum
{
	RHI_ATTACHMENT_STORE_OPERATION_DONT_CARE,
	RHI_ATTACHMENT_STORE_OPERATION_STORE,
...
typedef enum RHI_ShaderTypeEnum
{
  RHI_SHADER_TYPE_VERTEX,
  RHI_SHADER_TYPE_FRAGMENT,
  
  RHI_SHADER_TYPE_COUNT
} RHI_ShaderTypeEnum;

typedef U8 RHI_ShaderLanguage;
typedef enum RHI_ShaderLanguageEnum
{
  RHI_SHADER_LANGUAGE_SPIRV,

  RHI_SHADER_LANGUAGE_COUNT
...
{
  RHI_VERTEX_ATTRIBUTE_FORMAT_VEC2F32,
  RHI_VERTEX_ATTRIBUTE_FORMAT_VEC3F32,
	RHI_VERTEX_ATTRIBUTE_FORMAT_VEC4F32,

  RHI_VERTEX_ATTRIBUTE_FORMAT_VEC4I32,

  RHI_VERTEX_ATTRIBUTE_FORMAT_COUNT
...
typedef U8 RHI_BindingType;
typedef enum RHI_BindingTypeEnum
{
  RHI_BINDING_TYPE_UNIFORM_BUFFER,
  RHI_BINDING_TYPE_TEXTURE_2D,

  RHI_BINDING_TYPE_COUNT
} RHI_BindingTypeEnum;

typedef struct RHI_BindingInfo RHI_BindingInfo;
struct RHI_BindingInfo
{
  RHI_BindingType type;
  RHI_ShaderType  shader_type;
};

typedef U8 RHI_PipelineCullingMode;
typedef enum RHI_PipelineCullingModeEnum
{
  RHI_PIPELINE_CULLING_MODE_NONE,

  RHI_PIPELINE_CULLING_MODE_BACK_CW,
  RHI_PIPELINE_CULLING_MODE_BACK_CCW,

  RHI_PIPELINE_CULLING_MODE_COUNT
...
// -------------------------------------------------------------------
// State
typedef U8 RHI_RendererType;
typedef enum RHI_RendererTypeEnum
{
  RHI_RENDERER_TYPE_NONE,

  RHI_RENDERER_TYPE_VK,

  RHI_RENDERER_TYPE_COUNT
} RHI_RendererTypeEnum;
...
struct RHI_State
{
	...
	RHI_RendererType renderer_type;
} _r_state;

func B32 RHI_Init(RHI_RendererType type, OS_Window* window);
func B32 RHI_Shutdown(void);
