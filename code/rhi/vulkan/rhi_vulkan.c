...
RHI_VK_CreateShader(Arena* arena, RHI_ShaderCreateInfo* info) {
  ...
  result.code = (U8*)PushArena(arena, result.code_size * sizeof(U8));
	result.global_uniforms_count = info->global_uniforms_count;
...
func RHI_Shader
RHI_VK_CreateShaderNew(Arena* arena, RHI_ShaderCreateInfoNew* info) {
  RHI_Shader result = {0};
...
RHI_VK_CreateGraphicsPipeline(RHI_GraphicsPipelineCreateInfo* pipeline_info) {
  ...
  // --AlNov: @TODO Is this code could be compressed?
...
func RHI_GraphicsPipeline
RHI_VK_CreateGraphicsPipelineNew(RHI_GraphicsPipelineCreateInfo* pipeline_info) {
  ...
  if (pipeline.vertex_shader->arguments[0].size != 0) {
    push_constant_ranges[push_constants_count].stageFlags = VK_SHADER_STAGE_VERTEX_BIT;
    push_constant_ranges[push_constants_count].size = pipeline.vertex_shader->arguments[0].size;
    push_constants_count += 1;
  }
  if (pipeline.fragment_shader->arguments[0].size != 0) {
    push_constant_ranges[push_constants_count].stageFlags = VK_SHADER_STAGE_FRAGMENT_BIT;
    push_constant_ranges[push_constants_count].size = pipeline.fragment_shader->arguments[0].size;
    push_constants_count += 1;
  }
  VkPipelineLayoutCreateInfo layout_info = {
    .sType = VK_STRUCTURE_TYPE_PIPELINE_LAYOUT_CREATE_INFO,
...
func RHI_RenderPass*
RHI_VK_BeginRenderPassNew(RHI_CommandBuffer command_buffer, U32 color_targets_count, RHI_ColorTarget* color_targets, RHI_DepthStencilTarget* depth_stencil_target, RHI_Resource* resources, I32 resources_count) {
  return RHI_VK_BeginRenderPass(command_buffer, color_targets_count, color_targets, depth_stencil_target);
}
